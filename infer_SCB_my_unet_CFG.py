...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for index,(img1,img2,text) in enumerate(zip(img1_images,img2_images,text_list)):
            text='person, high quality, high resolution, detailed, realistic'
            ...
            for i, t in enumerate(timesteps):
                ...
                uncondition_down_block_res_samples = scb_net(
                    ...
                    cond_img=torch.concat([torch.zeros_like(img1_scb_cond).to(device) , torch.zeros_like(img1_scb_cond).to(device)], dim=1),
                    return_dict=False,
                    step=99999,
                )
