...
def parse_args(input_args=None):
    ...
    parser.add_argument("--save_root", default="Test_Results/With_refine_VAE_0310a/" )
...
if __name__ == "__main__":
    ...
    encoder_path = os.path.join(args.ckpt_dir, "encoder.pt")
    decoder_path = os.path.join(args.ckpt_dir, "decoder.pt")
    ...
    try:
        ...
        encoder.load_state_dict(torch.load(os.path.join(args.ckpt_dir, "encoder.pt"))['model'])
        decoder = CustomDecoder(config, vae.decoder).to(device)
    except:
    ...
    text_folder=os.path.join(args.img_path,'PSFusion_test_categories')
    img1_image_path = [
    ...
    with torch.no_grad():
        ...
        for index,(img1,img2,text) in enumerate(zip(img1_images,img2_images,text_list)):
            ...
            img1_scb_cond, feature1_list = encoder(img1, return_feature=True)
            img2_scb_cond, feature2_list = encoder(img2, return_feature=True)

            img1_scb_cond = vae.config.scaling_factor * torch.chunk(vae.quant_conv(img1_scb_cond), 2, dim=1)[0]
            img2_scb_cond = vae.config.scaling_factor * torch.chunk(vae.quant_conv(img2_scb_cond), 2, dim=1)[0]



            feature1_list = [feature / vae.config.scaling_factor for feature in feature1_list]
            feature2_list = [feature / vae.config.scaling_factor for feature in feature2_list]

            down_block_res_samples=vae_fuse_net(img1_scb_cond,img2_scb_cond)
            feature_list = [a + b for a, b in zip(feature1_list, feature2_list)]
            ...
            # post-process
            pred = decoder(vae.post_quant_conv(latents / vae.config.scaling_factor), feature_list)
