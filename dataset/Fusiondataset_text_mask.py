...
def make_VIFusion_train_dataset(args, accelerator):
    ...
    dataset['train'] = dataset['train'].map(lambda example: {'text': os.path.join(args.data_root, example['text'])})
    ...
    def preprocess_train(examples):
        ...
        # text_list = [text for text in examples[text_column]]
        text_list = [open(text_path, 'r', encoding='utf-8').read() for text_path in examples[text_column]]
        ...
        file_name_list=[os.path.basename(text_path).split('.')[0] for text_path in examples[text_column]]

        examples["label_list"] = label_list
        examples["img1_list"] = img1_list
        examples["img2_list"] = img2_list
        examples["text_list"] = text_list
        examples["file_name_list"] = file_name_list
...
def collate_fn(examples):
    ...
    text = [example["text_list"] for example in examples]
    file_name = [example["file_name_list"] for example in examples]
