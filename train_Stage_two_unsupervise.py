...
def parse_args(input_args=None):
    ...
    parser.add_argument("--output_dir", type=str, default="./results/Stage_two/temp")
    parser.add_argument("--ckpt_dir", type=str, default="results/VIFusion/Hadamard_0419a/pick-checkpoint-13500/", required=False, )
...
def main(args):
    ...
    # img1_tpb_net.eval()
    # img1_tpb_net.requires_grad_(False)
    ...
    # img2_tpb_net.eval()
    # img2_tpb_net.requires_grad_(False)
    ...
    # text2image_prompt_fusion_net.requires_grad_(False)

    # task_prompt_fusion_net= TaskPromptFusionNet(768, output_dim=768)
    # task_prompt_fusion_net.load_state_dict(
    #     torch.load(os.path.join(args.ckpt_dir, 'task_prompt_fusion_net.pt'), weights_only=True)['model'], strict=True)
    # task_prompt_fusion_net.eval()
    # task_prompt_fusion_net.requires_grad_(False)

    # vae_fuse_net=VAEFuseNet()
    # vae_fuse_net.load_state_dict(torch.load(os.path.join(args.ckpt_dir, 'vae_fuse_net.pt'), weights_only=True)['model'], strict=True)
    # vae_fuse_net.eval()
    # vae_fuse_net.requires_grad_(False)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(feature_fusion_module) and accelerator.accumulate(vae_decoder):
                ...
                # img1 = vae_image_processor.preprocess(batch["img1_pixel_values"], height=height, width=width).to(device=vae.device)
                # img = vae_image_processor.preprocess(batch["img2_pixel_values"], height=height, width=width).to(device=vae.device)
                # img1 = vae_image_processor.preprocess(batch["label_values"], height=height, width=width).to(device=vae.device)
                ...
                # visualization_tensor(z1)
                # visualization_tensor(z2)
                ...
                for file_name in file_name_list:
                    latents_list.append(torch.load(os.path.join(f'./Test_Results/Hadamard_0419a_latent/{file_name}_latents.pt')))
                latents=torch.concat(latents_list,dim=0).to(accelerator.device)
                # visualization_tensor(latents)
                ...
                fusion_loss,_,_,_=Fusion_loss(img2_image,img1_image,rec,device=img1_image.device,)
                # loss = F.mse_loss(label_image.float(), rec.float(), reduction="mean")
                accelerator.backward(fusion_loss)
            ...
            logs = {"loss": fusion_loss.detach().item(),"lr": lr_scheduler.get_last_lr()[0]}
