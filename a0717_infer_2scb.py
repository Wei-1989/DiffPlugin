...
def parse_args(input_args=None):
    ...
    parser.add_argument("--ckpt_dir", type=str, default="results/VIFusion/0717a/checkpoint-13500/", required=False, )
    ...
    parser.add_argument("--use_condition_guidance", type=float, default=1.5,)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for index, (img1, img2, text) in enumerate(zip(img1_images, img2_images, text_list)):
            text='people'
            # text+=', '+args.added_prompt if args.added_prompt else text
            ...
            if use_condition_guidance:
                no_condition_text_token = tokenizer(
                ...
                no_condition_text_features = text_encoder(no_condition_text_token.input_ids)[0]
            ...
            for i, t in enumerate(timesteps):
                if use_condition_guidance:
                    down_block_res_samples = scb_net(
                        ...
                        return_dict=False,
                        use_condition_guidance=False,
                        time_step=i,
                    )
                    no_condition_down_block_res_samples = scb_net(
                        timestep=t,
                        encoder_hidden_states=no_condition_text_features,
                        cond_img=torch.concat([torch.zeros_like(img1_scb_cond,dtype=img1_scb_cond.dtype,device=img1_scb_cond.device), torch.zeros_like(img1_scb_cond,dtype=img1_scb_cond.dtype,device=img1_scb_cond.device)],
                                              dim=1),
                        return_dict=False,
                        use_condition_guidance=False,
                        time_step=i,
                    )
                    down_block_res_samples=[torch.cat([u,v],dim=0) for u,v in zip(no_condition_down_block_res_samples,down_block_res_samples)]
                    # for ii in down_block_res_samples:
                    #     visualize_tensor(ii)
                    latent_input = torch.cat([latents] * 2)
                    encoder_input = torch.cat([no_condition_text_features, text_features])
                    noise_pred = unet(
                        latent_input,
                        t,
