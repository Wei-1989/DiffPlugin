...
import json
from modules import VAEFeatureFusionModule
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--save_root", default="Test_Results/Finetune_VAE_0310a" )
...
if __name__ == "__main__":
    ...
    encoder.load_state_dict(torch.load(os.path.join(args.ckpt_dir, "encoder.pt"),weights_only=True)['model'],strict=True)
    decoder=CustomDecoder(config,vae.decoder).to(device)
    decoder.load_state_dict(torch.load(os.path.join(args.ckpt_dir, "decoder.pt"),weights_only=True)['model'],strict=True)
    ...
    with torch.no_grad():
        ...
        for index,(img1,img2,text) in enumerate(zip(img1_images,img2_images,text_list)):
            ...
            visualization_tensor(img1_prompt_guidance.reshape(1,-1,16,16),reduced_dim=1)
            visualization_tensor(image_encoder(img1_normalized_pixel_values, output_attentions=True, output_hidden_states=True).hidden_states[1][:,
                                 1:, :].reshape(1,-1,16,16),reduced_dim=1)
            visualization_tensor(img2_prompt_guidance.reshape(1,-1,16,16))
            visualization_tensor(
                image_encoder(img2_normalized_pixel_values, output_attentions=True, output_hidden_states=True).reshape(1, -1, 16, 16))

            fusion_prompt_guidance = task_prompt_fusion_net(img1_prompt_guidance, img2_prompt_guidance)

            visualization_tensor(fusion_prompt_guidance.reshape(1, -1, 16, 16))
            ...
            z2 = encoder(img2_preprocess, return_feature=False)
            ...
            rec = decoder(vae.post_quant_conv(latents / vae.config.scaling_factor), feature1_list)
