...
from val_VIF_multiFusion_attnBeforeFuse import log_validation
from utils import save_random_states
from dataset.Fusiondataset_withText  import make_VIFusion_train_dataset, collate_fn
from modules import TPBNet,PromptFusionModule,VAEFeatureFusionModule,TICrossModalFusionModule
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--output_dir",type=str,default="./results/VIFusion/multiFusion_attnBeforeFuse-0304a",)
...
def main(args):
    ...
    tiCrossModal_fusion_net=TICrossModalFusionModule(768)
    vae_fuse_net=VAEFeatureFusionModule()
    ...
    params_to_optimize.append({'params': vae_fuse_net.parameters(), 'lr': args.learning_rate})
    params_to_optimize.append({'params': tiCrossModal_fusion_net.parameters(), 'lr': args.learning_rate})
    ...
    optimizer, train_dataloader, lr_scheduler, img1_tpb_net,img2_tpb_net,tiCrossModal_fusion_net,vae_fuse_net= accelerator.prepare(
        optimizer,train_dataloader,lr_scheduler, img1_tpb_net,img2_tpb_net,tiCrossModal_fusion_net,vae_fuse_net)
    ...
    vae_fuse_net.to(accelerator.device, dtype=weight_dtype)
    tiCrossModal_fusion_net.to(accelerator.device, dtype=weight_dtype)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with ((accelerator.accumulate(img1_tpb_net)and accelerator.accumulate(img2_tpb_net)) and accelerator.accumulate(tiCrossModal_fusion_net) and
                  accelerator.accumulate(vae_fuse_net)):
                ...
                text=tokenizer(batch["text"], return_tensors="pt", padding="max_length", truncation=True).to(accelerator.device)
                ...
                if accelerator.sync_gradients:
                    ...
                    accelerator.clip_grad_norm_(vae_fuse_net.parameters(), args.max_grad_norm)
                    accelerator.clip_grad_norm_(tiCrossModal_fusion_net.parameters(), args.max_grad_norm)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if (global_step % args.checkpointing_steps==0 and global_step >= 40000) or global_step == 1:
                        ...
                        if global_step != 1:
                            ...
                            save({'model': vae_fuse_net.state_dict()}, os.path.join(save_path, 'vae_fuse_net.pt'))
                            logger.info(f"Saved vae_fuse_net to {save_path}")


                            save({'model': tiCrossModal_fusion_net.state_dict()}, os.path.join(save_path, 'tiCrossModal_fusion_net.pt'))
                            logger.info(f"Saved tiCrossModal_fusion_net to {save_path}")
                    ...
                    if global_step % args.validation_steps == 0:
                        ...
                        log_validation(logger, vae, unet, image_encoder, text_encoder,tokenizer,vae_fuse_net, img1_tpb_net,img2_tpb_net,tiCrossModal_fusion_net, args, accelerator, global_step)
