...
from modules import MultiScaleFeatureFusionModule
import transformers
from transformers import CLIPVisionModel
from transformers.image_utils import OPENAI_CLIP_MEAN, OPENAI_CLIP_STD

from Finetune_dual_VAE_val import log_validation
from utils import save_random_states
from dataset.Finetune_VAE_datasets  import make_VIFusion_train_dataset, collate_fn
from modules import TPBNet, SCBNet,PromptFusionModule,FeatureFusionModule
from utils import  get_latest_checkpoint, save_args, code_backup
from loss.losses import *
from loss import ColorLoss
from modules import CustomEncoder,CustomDecoder
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--output_dir",type=str,default="./results/VAE/Finetune_dual_VAE_0311a",)
    ...
    parser.add_argument("--train_data_file_list",type=str,default='data/train/finetune_VAE.csv',)
...
def main(args):
    ...
    encoder = CustomEncoder(config, vae.encoder)
    decoder = CustomDecoder(config, vae.decoder)

    feature_fusion_module = MultiScaleFeatureFusionModule([128,128,256,512,512])
    # -------
    ...
    params_to_optimize = []
    # params_to_optimize.append({'params': encoder.parameters(), 'lr': args.learning_rate})
    params_to_optimize.append({"params": [p for p in decoder.refine_module_list.parameters() if p.requires_grad], "lr": args.learning_rate},)
    params_to_optimize.append({"params": [p for p in decoder.final_refine.parameters() if p.requires_grad], "lr": args.learning_rate},)
    ...
    optimizer, train_dataloader, lr_scheduler,decoder,feature_fusion_module= accelerator.prepare(optimizer,train_dataloader,lr_scheduler,decoder,feature_fusion_module)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(vae.encoder):
                width,height=batch["img1"][0].size
                img1 = vae_image_processor.preprocess(batch["img1"], height=height, width=width).to(device=vae.device)
                img2 = vae_image_processor.preprocess(batch["img2"], height=height, width=width).to(device=vae.device)
                label = vae_image_processor.preprocess(batch["label"], height=height, width=width).to(device=vae.device)
                pt_file = batch["pt"]
                pt_list=[]
                for i in pt_file:
                    i=torch.load(i).to(dtype=weight_dtype).to(accelerator.device)
                    pt_list.append(i)
                pt=torch.cat(pt_list,dim=0)

                _,feature1_list=encoder(img1,return_feature=True)
                _,feature2_list=encoder(img2,return_feature=True)

                # z1 = vae.config.scaling_factor * torch.chunk(vae.quant_conv(z1), 2, dim=1)[0]
                # z2 = vae.config.scaling_factor * torch.chunk(vae.quant_conv(z2), 2, dim=1)[0]
                ...
                rec = decoder(vae.post_quant_conv(pt / vae.config.scaling_factor),feature_list)
                # 梯度损失 ----------------------------
                # sobelConv = Sobelxy(vae.device)
                # img_grad_x, img_grad_y = sobelConv(img_image)
                # rec_grad_x, rec_grad_y = sobelConv(rec)
                # end 梯度损失----------------------
                # grad_loss = F.l1_loss(img_grad_x,rec_grad_x) + F.l1_loss(img_grad_y, rec_grad_y)
                pixel_loss = F.mse_loss(label.float(), rec.float(), reduction="mean")
                # color_loss= Color_Loss(label,rec)
                loss=pixel_loss
                ...
                if accelerator.sync_gradients:
                    accelerator.clip_grad_norm_(vae.encoder.parameters(), args.max_grad_norm)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if ((global_step % args.checkpointing_steps==0 and global_step >= 5000) or global_step == 1 or global_step ==
                        ...
                        if global_step != 1:
                            ...
                            save({'model': encoder.state_dict()}, os.path.join(save_path, 'encoder.pt'))
                            logger.info(f"Saved encoder to {save_path}")

                            save({'model': decoder.state_dict()}, os.path.join(save_path, 'decoder.pt'))
                    ...
                    if global_step % args.validation_steps == 0:
                        log_validation(logger, vae,encoder,decoder,feature_fusion_module, args, accelerator, global_step)
                        encoder.train()
                        decoder.train()
