...
def log_validation(logger, vae, unet, image_encoder,  scb_net, img1_tpb_net,img2_tpb_net,
    ...
    image_logs = validation(args, vae, unet, image_encoder,  scb_net, img1_tpb_net,img2_tpb_net,task_prompt_fusion_net, noise_scheduler, args.validation_image, logger=logger)
    ...
    # concat_imgs(img_list_for_save).save(os.path.join(args.output_dir, "visuals", str(step) + '.png'), target_size=512, target_dim=1)
    concat_imgs(img_list_for_save,target_size=img_list_for_save[0].size).save(os.path.join(args.output_dir, "visuals", str(step) + '.png'),
                                                           target_size=img_list_for_save[0].size,target_dim=1)

    return image_logs



def validation(args, vae, unet, image_encoder, scb_net, img1_tpb_net,img2_tpb_net,task_prompt_fusion_net, noise_scheduler, validation_images, logger=None):
    ...
    for index, (img1,img2) in enumerate(zip(img1_image_path,img2_image_path)):
        ...
        with torch.no_grad():
            img1_input = clip_image_processor(images=img1, return_tensors="pt").pixel_values.to(device=vae.device) # 3,224,244
            ...
            # img1_preprocess = vae_image_processor.preprocess(img1, height=args.resolution, width=args.resolution).to(device=vae.device)
            # img2_preprocess = vae_image_processor.preprocess(img2, height=args.resolution, width=args.resolution).to(device=vae.device)  # image now is
            # tensor in [-1,1]
            img1_preprocess = vae_image_processor.preprocess(img1, height=new_height, width=new_width).to(device=vae.device) # 3,512,680
            img2_preprocess = vae_image_processor.preprocess(img2, height=new_height, width=new_width).to(device=vae.device)
            img1_cond=vae.config.scaling_factor * torch.chunk(vae.quant_conv(vae.encoder(img1_preprocess)), 2, dim=1)[0] #4,64,85
            ...
            for _, t in enumerate(timesteps):
                ...
                down_block_res_samples = scb_net(latents, t, encoder_hidden_states=fusion_prompt_guidance, cond_img=img1_cond, return_dict=False,) # 320,64,85
                noise_pred = unet(latents, t, encoder_hidden_states=fusion_prompt_guidance, down_block_additional_residuals=down_block_res_samples).sample
