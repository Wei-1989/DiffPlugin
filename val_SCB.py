...
def validation(args, vae, unet, clip_image_encoder, text_encoder, tokenizer,textAdapter, img1_tpb_net, img2_tpb_net,scb_net, task_prompt_fusion_net,
    ...
    img1_image_folder = os.path.join(validation_images, 'vi')
    img2_image_folder = os.path.join(validation_images, 'ir')
    text_folder = os.path.join(validation_images, 'gt_category_no_commas')
    # 获取文件夹中的所有文件
    img1_image_path = [
    ...
    for index, (img1, img2, text) in enumerate(zip(img1_image_path, img2_image_path, text_path)):
        ...
        with torch.no_grad():
            ...
            text = tokenizer(content, return_tensors="pt", padding=False, truncation=True).to(vae.device)
            ...
            for _, t in enumerate(timesteps):
                ...
                # visualization_tensor(noise_pred[0])
