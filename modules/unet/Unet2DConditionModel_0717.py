...
from modules.My_CrossAttnDownBlock2D.CrossAttnUpBlock_0717 import MyCrossAttnUpBlock2D
...
class FeatureFusion(nn.Module):
    ...
    def forward(self, feat1, feat2):
        ...
        Returns:
            fused_feat: Weighted sum of feat1 and feat2
        """
        w1 = self.weight_gen1(feat1)  # Shape: (B, 1, H, W)
        w2 = self.weight_gen2(feat2)  # Shape: (B, 1, H, W)
        ...
        # Weighted sum
        fused_feat = feat1 * w1_norm + feat2 * w2_norm
        return fused_feat
class My_Unet(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def __init__(
        ...
        for i, down_block_type in enumerate(down_block_types):
            ...
            if down_block_type in ["DownBlock2D", "CrossAttnDownBlock2D", "ResnetDownsampleBlock2D",
                ...
                module_path = f'modules.My_CrossAttnDownBlock2D.{down_block_type}'
                module = importlib.import_module(module_path)
                My_CrossAttnDownBlock2D = getattr(module, 'My_CrossAttnDownBlock2D')
                down_block = My_CrossAttnDownBlock2D(
                    num_layers=layers_per_block[i],
                    in_channels=input_channel,
                    out_channels=output_channel,
                    temb_channels=blocks_time_embed_dim,
    ...
    def forward(
        ...
        for idx,downsample_block in enumerate(self.down_blocks):
            if hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention:
                sample, res_samples = downsample_block(
                    ...
                    time_step=timestep_cond,
                    idx_CrossAttnDownBlock=idx,
                )
            else:
        ...
        if down_block_additional_residuals is not None:
            ...
            for i,(down_block_res_sample, down_block_additional_residual,fuse_module) in enumerate(zip(down_block_res_samples,
                                                                                                   down_block_additional_residuals,
                                                                                                       self.fusion_weight)):
                # if time_step==49:
                #     for ori,add in zip(down_block_res_sample,down_block_additional_residual):
                #         visualize_tensor(ori)
                #         visualize_tensor(add)
                down_block_res_sample = fuse_module(down_block_res_sample, down_block_additional_residual)
                # down_block_res_sample = down_block_res_sample + down_block_additional_residual

                new_down_block_res_samples = new_down_block_res_samples + (down_block_res_sample,)

            # for i,(down_block_res_sample, down_block_additional_residual) in enumerate(zip(down_block_res_samples, down_block_additional_residuals)):
            #     if i<1:
            #         visualization_tensor(down_block_res_sample[0])
            #         visualization_tensor(down_block_additional_residual[0])
            #     down_block_res_sample = down_block_res_sample+down_block_additional_residual
            #     if i<1:
            #         visualization_tensor(down_block_res_sample[0])
            #     new_down_block_res_samples = new_down_block_res_samples + (down_block_res_sample,)
