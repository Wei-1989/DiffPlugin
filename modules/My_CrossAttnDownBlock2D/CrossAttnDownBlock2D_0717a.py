...
from Myutils.visualize2 import visualize_tensor,visualize_token
from diffusers.models.resnet import ResnetBlock2D,Downsample2D
from diffusers.models import DualTransformer2DModel
import os
from modules.register import CADB_BLOCKS
...
class TGFM(nn.Module):
    ...
    def __init__(
        ...
        if not self.only_cross_attention:
            ...
            self.to_v_Hadamard = nn.Linear(query_dim, inner_dim, bias=bias)
        else:
            self.to_k = None
            self.to_v_Hadamard = None
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None, temb=None,return_attn_map=False):
        residual = hidden_states
        ...
        value = self.to_v_Hadamard(hidden_states)
        ...
        d_k = query.shape[-1]  # head_dim
        scores = torch.matmul(query, key.transpose(-2, -1)) / d_k ** 0.5
        # visualization_token(scores,h_w_ratio=0.75,num_token=10)
...
class My_BasicTransformerBlock(BasicTransformerBlock):
    def __init__(self, *args, **kwargs):
        ...
        double_self_attention= kwargs.get("double_self_attention",False)
        num_attention_heads=kwargs.get("num_attention_heads")
        attention_head_dim=kwargs.get("attention_head_dim")
    ...
    def forward(
        ...
        time_step=None,
        idx_CrossAttnDownBlock=None,
        idx_Transformer2D=None,
        idx_BasicTransformerBlock=None,
        type="random",
    ):
        ...
        if self.attn2 is not None and encoder_hidden_states is not None:
            ...
            # visualize_token(norm_hidden_states)
            cross_attn_output = Cross_Attn(self.attn2, norm_hidden_states, encoder_hidden_states=encoder_hidden_states,
                                                    attention_mask=encoder_attention_mask)
            # visualize_token(cross_attn_output)
            cross_attn_output=self.TGFM(cross_attn_output,encoder_hidden_states=encoder_hidden_states,
                                                       attention_mask=encoder_attention_mask,return_attn_map=return_attn_map)


            if return_attn_map:
                cross_attn_output, attn_weight=cross_attn_output
                if time_step == 49 and idx_CrossAttnDownBlock == 1 and idx_Transformer2D == 0 and idx_BasicTransformerBlock == 0:
                    visualize_token(attn_weight,type='BHTT',num_token=6)
    ...
    def forward(
        ...
        for idx,block in enumerate(self.transformer_blocks):
            hidden_states = block(
                ...
                time_step=time_step,
                idx_CrossAttnDownBlock=idx_CrossAttnDownBlock,
                idx_Transformer2D=idx_Transformer2D,
                idx_BasicTransformerBlock=idx,
                type=type,
            )
            basicTrans_idx+=1
            if return_attn_map:
