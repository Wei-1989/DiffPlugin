...
def Cross_Attn(attn,hidden_states,encoder_hidden_states=None,attention_mask=None,temb=None,return_attn=False):
    residual = hidden_states
    if attn.spatial_norm is not None:
        hidden_states = attn.spatial_norm(hidden_states, temb)
    ...
    # attn_map=F.softmax((query @ key.transpose(-2, -1)),dim=-1)
    attn_map=query @ key.transpose(-2, -1)
    ...
    return hidden_states
def Cross_Attn_Hadamard(attn,hidden_states,encoder_hidden_states=None,attention_mask=None,temb=None,return_attn_map=False):
    residual = hidden_states
    if attn.spatial_norm is not None:
        hidden_states = attn.spatial_norm(hidden_states, temb)
...
class My_BasicTransformerBlock(BasicTransformerBlock):
    ...
    def forward(
        ...
        # if step==0 and CrossADB_idx==0 and TransformerBlock_idx in [0] and basicTrans_idx==0:
        ...
        if self.attn2 is not None and encoder_hidden_states is not None:
            ...
            if return_attn_map:
                cross_attn_output,attn_weight=Cross_Attn_Hadamard(self.attn2,norm_hidden_states,encoder_hidden_states=encoder_hidden_states,
                                                       attention_mask=encoder_attention_mask,return_attn_map=return_attn_map)
                if step==4 and CrossADB_idx==0 and TransformerBlock_idx ==0 and basicTrans_idx==0:
                    visualization_token(attn_weight,cmap='viridis')

            else:
                cross_attn_output = Cross_Attn_Hadamard(self.attn2, norm_hidden_states, encoder_hidden_states=encoder_hidden_states,
                                                                     attention_mask=encoder_attention_mask,return_attn_map=return_attn_map)
...
class My_CrossAttnDownBlock2D(nn.Module):
    ...
    def forward(
        ...
        if self.downsamplers is not None:
            ...
            output_states = output_states + (hidden_states,)
        if return_attn_map:
            return hidden_states,output_states,attn_map_list
