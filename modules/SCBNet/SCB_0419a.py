...
# from modules.My_CrossAttnDownBlock2D import My_CrossAttnDownBlock2D_0419a as My_CrossAttnDownBlock2D
class SCBNet(ModelMixin, ConfigMixin):
    ...
    def forward(
            ...
            use_condition_guidance: bool = False,
        ...
        sample=self.init_fusion(torch.concat([vi_feature,ir_feature],dim=1))
        if use_condition_guidance:
            # sample=torch.cat([torch.zeros_like(sample,device=sample.device,dtype=sample.dtype),sample],dim=0)
        ...
        for i,downsample_block in enumerate(self.down_blocks):
            if hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention:
                sample = downsample_block(
                    ...
                    time_step=time_step,
                    idx_CrossAttnDownBlock=i,
                    type=type,
                )
                if return_attn_map:
                    sample, res_samples,attn_map = sample
                    attn_map_list.append(attn_map)
                else:
                    sample,res_samples=sample
        ...
        if not return_dict:
            if return_attn_map:
                return down_block_res_samples, attn_map_list
            else:
