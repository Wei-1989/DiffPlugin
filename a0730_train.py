...
def parse_args(input_args=None):
    ...
    parser.add_argument("--output_dir", type=str, default="./results/VIFusion/0730a",)
    parser.add_argument("--num_train_epochs", type=int, default=150)
    parser.add_argument("--checkpointing_steps", type=int, default=500,)
    parser.add_argument("--validation_steps", type=int, default=500,)
    parser.add_argument("--start_validation_steps", type=int, default=50,)
    parser.add_argument("--train_data_file_list", type=str, default='data/train/PSF_gt_category_with_commas.csv',)
    ...
    parser.add_argument("--use_vae_refine", type=bool, default=True, help="use vae refine or not, default True")
...
def main(args):
    ...
    # 5. 加载最终权重
    unet.load_state_dict(my_state_dict, strict=False)
    del pretrained_unet# 释放原来权重内存

    # 冻结部分unet权重
    for param in unet.parameters():
        param.requires_grad = False
    for module in unet.fusion_weight:
    ...
    params_to_optimize.append({'params': unet.fusion_weight.parameters(), 'lr': args.learning_rate})
    params_to_optimize.append({'params': [param for name, param in unet.named_parameters() if "TGFM" in name], 'lr': args.learning_rate})
    ...
    (optimizer, train_dataloader, lr_scheduler,scb_net) = accelerator.prepare(
        optimizer,
        train_dataloader,
        lr_scheduler,
        scb_net,)
    ...
    if accelerator.is_main_process:
        ...
        tracker_config.pop("ctrl_down_block_types")
        tracker_config.pop("unet_down_block_types")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(scb_net):
                ...
                text_token = tokenizer(batch["text"],return_offsets_mapping=True, return_tensors="pt", padding='max_length', truncation=True).to(accelerator.device)
                offsets=text_token["offset_mapping"]
                ...
                # 是否有attn返回
                if isinstance(down_block_res_samples,tuple):
                    down_block_res_samples, attn_map_list = down_block_res_samples
                ...
                mask_folder = './TrainData/VIFusion/MSRS/visual_label'
                mask_folder_list=[os.path.join(mask_folder,file_name) for file_name in file_name_list]
                mask_list=[]
                for mask_folder in mask_folder_list:
                    mask_list.append([os.path.join(mask_folder,file) for file in os.listdir(mask_folder) if os.path.splitext(file)[0].isdigit()])

                attentionMapLoss = AttentionMapLoss()
                attention_map_loss = attentionMapLoss(mask_list,attn_map_list,offsets,batch["text"],text_token)

                loss = noise_loss +100*attention_map_loss
                accelerator.backward(loss)

                if accelerator.sync_gradients:
                    accelerator.clip_grad_norm_(scb_net.parameters(), args.max_grad_norm)
                    accelerator.clip_grad_norm_(unet.fusion_weight.parameters(), args.max_grad_norm)
                    accelerator.clip_grad_norm_([param for name, param in unet.named_parameters() if "TGFM" in name], args.max_grad_norm)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0 or global_step == 1 or global_step==len(train_dataloader)*args.num_train_epochs:
                        ...
                        if global_step != 1:
                            ...
                            save({'model': unet.fusion_weight.state_dict()}, os.path.join(save_path, 'unet_fusion_weight.pt'))
                            tgfm_state_dict = {
                                name: param for name, param in unet.state_dict().items()
                                if "TGFM" in name
                            }
                            torch.save({'model': tgfm_state_dict}, os.path.join(save_path, "unet_TGFM_only.pt"))

                        logger.info(f"Saved state to {save_path}")
                    if (global_step < 500 and global_step % args.start_validation_steps == 0) or \
                            (global_step >= 500 and global_step % args.validation_steps == 0):
                        log_validation(
                            logger=logger,
                            vae=vae,
                            unet=unet,
                        ...
                        scb_net.train()
                        unet.fusion_weight.train()
                        unet.train()

            # logs = {"noise_loss": noise_loss.detach().item(),"attn_loss": attention_map_loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0]}
            logs = {"noise_loss": noise_loss.detach().item(), "refine_loss":
                attention_map_loss.detach().item(),}
