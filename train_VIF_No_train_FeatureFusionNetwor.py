...
def parse_args(input_args=None):
    ...
    parser.add_argument("--output_dir",type=str,default="./results/VIFusion/WholeNet-no-train-FusionNetwork",)
...
def main(args):
    ...
    feature_fusion_net.load_state_dict(torch.load('./results/VIFusion/FusionNetwork-NativeConv/OneConv-checkpoint-25400/feature_fusion_net.pt', weights_only=True)['model'], strict=True)
    ...
    optimizer, train_dataloader, lr_scheduler, scb_net, img1_tpb_net,img2_tpb_net,task_prompt_fusion_net= accelerator.prepare(optimizer,train_dataloader,
                                                                                                             lr_scheduler, scb_net, img1_tpb_net,
                                                                                                             img2_tpb_net,task_prompt_fusion_net)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with ((accelerator.accumulate(scb_net) and accelerator.accumulate(img1_tpb_net)and accelerator.accumulate(img2_tpb_net))and
                  accelerator.accumulate(task_prompt_fusion_net)):
