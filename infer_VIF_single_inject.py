...
from modules import FeatureFusionModule
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--ckpt_dir", type=str, default="results/VIFusion/diff-plugin/checkpoint-200/", required=False,)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        # TPB
        for index,(img1,img2) in enumerate(zip(img1_images,img2_images)):
            img1_clip_visual_input = clip_image_processor(images=img1, return_tensors="pt").pixel_values.to(device=vae.device) #3,224,224
            ...
            img1_scb_cond = vae.config.scaling_factor * torch.chunk(vae.quant_conv(vae.encoder(img1)), 2, dim=1)[0] #4,64,85
            ...
            for i, t in enumerate(timesteps):
                ...
                # SCB
                down_block_res_samples = scb_net(latents,t,encoder_hidden_states=fusion_prompt_guidance,cond_img=img1_scb_cond,return_dict=False,)
                # diffusion unet
            ...
            save_.save(os.path.join('./temp_results/',os.path.basename(img1_image_path[index])))
            print(f'--------{index} done-----------')
    print('--------all done-----------')
