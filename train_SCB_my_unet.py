...
from loss import AttentionMapLoss

from modules import TPBNet, TaskPromptFusionNet, Text2ImagePromptFusionModule,SCBNet,MyUnet,TextAdapter
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--output_dir", type=str, default="./results/VIFusion/Abl_unet_0.3_0.7",)
    parser.add_argument("--num_train_epochs", type=int, default=100)
    parser.add_argument("--checkpointing_steps", type=int, default=500,)
    parser.add_argument("--validation_steps", type=int, default=500,)
    parser.add_argument("--train_data_file_list", type=str, default='data/train/PSF_gt_category_no_commas.csv',)
    ...
    parser.add_argument("--num_inference_steps", type=int, default=5, help=("diffusion steps for inference process"), )
...
def main(args):
    ...
    # 4. 初始化新增模块（如 fusion_weight）
    # for module in unet.fusion_weight:
    #     # 初始化每个 FeatureFusion 的子模块
    #     for submodule in module.children():  # 遍历 weight_gen1 和 weight_gen2
    #         if isinstance(submodule, nn.Sequential):
    #             for layer in submodule:
    #                 if isinstance(layer, nn.Conv2d):
    #                     torch.nn.init.xavier_uniform_(layer.weight)
    #                     if layer.bias is not None:
    #                         torch.nn.init.zeros_(layer.bias)  # 可选：初始化偏置
    ...
    (optimizer, train_dataloader, lr_scheduler,textAdapter, img1_tpb_net, img2_tpb_net, text2image_prompt_fusion_net, task_prompt_fusion_net,
     scb_net) = accelerator.prepare(
        optimizer,
        train_dataloader,
        lr_scheduler,
        textAdapter,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with ((accelerator.accumulate(img1_tpb_net) and accelerator.accumulate(img2_tpb_net)) and
                  accelerator.accumulate(task_prompt_fusion_net) and accelerator.accumulate(text2image_prompt_fusion_net) and
                  accelerator.accumulate(scb_net) and accelerator.accumulate(textAdapter)):
                # flag=False
                # for t in batch['text']:
                ...
                # attention_map_loss = attentionMapLoss(mask_list,attn_map_list)

                # loss = noise_loss + refine_loss+100*attention_map_loss
