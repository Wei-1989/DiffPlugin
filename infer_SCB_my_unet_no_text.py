...
def parse_args(input_args=None):
    ...
    parser.add_argument("--img_path", type=str, default="../datasets/FMB/train")
    parser.add_argument("--save_root", default="Test_Results/MyUnet_0424a_FMB_train")
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for index,(img1,img2) in enumerate(zip(img1_images,img2_images)):
            ...
            # text_token = tokenizer(text, return_tensors="pt", padding=False, truncation=True).to(device=vae.device)
            # temp = text_encoder(**text_token, output_attentions=True, output_hidden_states=True).last_hidden_state
            # print(torch.norm(temp.mean(1) - text_features.mean(1), p=2))

            # visualization_tensor(text_features,batch_dim_exit=True)
            # text_features = textAdapter(text_features)
