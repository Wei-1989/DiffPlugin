...
def log_validation(logger, vae, encoder,decoder, args, accelerator, step):
    ...
    # feature_fusion_module = accelerator.unwrap_model(feature_fusion_module)
    ...
    image_logs = validation(args, vae, encoder,decoder, image_encoder,args.validation_image, logger=logger)
...
def validation(args, vae, encoder,decoder,feature_fusion_module,unet, image_encoder,text_encoder,tokenizer, vae_fuse_net, img1_tpb_net, img2_tpb_net,
    ...
    # 只要特定文件-----------------
    target_filenames = {"01458D","00081D","00111D","00123D","00209D"}
    ...
    for index, (img1, img2,text) in enumerate(zip(img1_image_path, img2_image_path,text_path)):
        ...
        with torch.no_grad():
            ...
            pred_pil = vae_image_processor.postprocess(rec, output_type='pil')[0]
