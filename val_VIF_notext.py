...
def log_validation(logger, vae, unet, image_encoder, scb_net, img1_tpb_net, img2_tpb_net, feature_fusion_net,
    ...
    text2image_prompt_fusion_net = accelerator.unwrap_model(text2image_prompt_fusion_net)
    ...
    image_logs = validation(args, vae, unet, image_encoder, scb_net, img1_tpb_net, img2_tpb_net, feature_fusion_net,
                            text2image_prompt_fusion_net,
    ...
    for tracker in accelerator.trackers:
        ...
        for log in image_logs:
            ...
            formatted_images.append(np.asarray(fuse))
    ...
    fuse_save_path = os.path.join(args.output_dir, "visuals/fuse")
    ...
    concat_imgs(fuse_list_for_save, target_size=fuse_list_for_save[0].size).save(os.path.join(fuse_save_path, str(step) + '.png'),
                                                                                 target_size=fuse_list_for_save[0].size, target_dim=1)

    return image_logs


def validation(args, vae, unet, image_encoder, scb_net, img1_tpb_net, img2_tpb_net, feature_fusion_net,
               text2image_prompt_fusion_net,
               task_prompt_fusion_net, noise_scheduler,
    ...
    random_indices = random.sample(range(0, len(img1_image_path)), 5)
