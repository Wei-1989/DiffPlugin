...
from loss.Fusion_losses import *
...
class HistogramColorLoss(nn.Module):
    ...
    def compute_histogram(self, img):
        ...
        for i in range(C):  # 分别计算 R, G, B 通道的直方图
            hist = torch.histc(img[:, i, :], bins=self.bins, min=0.0, max=1.0)
            hist = hist / hist.sum()  # 归一化
            histograms.append(hist)

        return torch.stack(histograms, dim=1)  # [B, C, bins]

    def forward(self, enhanced_img, original_img):
        """ 计算增强图像和原始图像之间的颜色直方图损失 """
        enhanced_hist = self.compute_histogram(enhanced_img)
        original_hist = self.compute_histogram(original_img)

        # 使用 KL 散度衡量两个分布之间的差异
        loss = F.kl_div(enhanced_hist.log(), original_hist, reduction='batchmean')
        return loss
...
class Sobelxy(nn.Module):
    ...
    def __init__(self, device):
        ...
        kernely = [[1, 2, 1],
                   ...
                   [-1, -2, -1]]
        # 这里不行就采用expend_dims
        kernelx = torch.FloatTensor(kernelx).repeat(1,3,1,1)
        kernely = torch.FloatTensor(kernely).repeat(1,3,1,1)
...
if __name__ == '__main__':
    function=Sobelxy(device='cuda')
    data1=torch.tensor([1,3],dtype=torch.float32)
    data2=torch.tensor([1,4],dtype=torch.float32)

    loss=F.l1_loss(data1,data2)
    print(loss)
