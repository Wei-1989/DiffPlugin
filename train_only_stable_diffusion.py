...
from modules import TPBNet, TaskPromptFusionNet, Text2ImagePromptFusionModule,SCBNet,TextAdapter,MyUnet
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--output_dir", type=str, default="./results/VIFusion/temp",)
...
def main(args):
    ...
    unet = MyUnet.from_pretrained("./pretrained-large-modal/unet", subfolder="unet", revision=None)
    ...
    (optimizer, train_dataloader, lr_scheduler,textAdapter, img1_tpb_net, img2_tpb_net, text2image_prompt_fusion_net, task_prompt_fusion_net,) = accelerator.prepare(
        optimizer,
        train_dataloader,
        lr_scheduler,
        textAdapter,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with ((accelerator.accumulate(img1_tpb_net) and accelerator.accumulate(img2_tpb_net)) and
                  accelerator.accumulate(task_prompt_fusion_net) and accelerator.accumulate(text2image_prompt_fusion_net) and accelerator.accumulate(textAdapter)):
                # flag=False
                # for t in batch['text']:
