...
from val_VIF import log_validation
from utils import save_random_states
from dataset.Fusiondataset  import make_VIFusion_train_dataset, collate_fn
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--output_dir",type=str,default="./results/VIFusion/WholeNetwork-CNN-Attention",)
    ...
    parser.add_argument("--train_data_file_list",type=str,default='data/train/vi_fusion.csv',)
    parser.add_argument("--validation_image",type=str,default="data/val/vi_fusion",help="List of validation image paths")
...
def main(args):
    ...
    # text="The image features a woman walking down a street at night, carrying a backpack. She is wearing a black coat and appears to be the main subject of the scene. There are also two other people in the background, one of them carrying a handbag. "
    # text_input = tokenizer(text, return_tensors="pt", padding="max_length", max_length=77, truncation=True)
    # features = text_encoder(**text_input, output_attentions=True, output_hidden_states=True).last_hidden_state
    ...
    feature_fusion_net = FeatureFusionModule(4,4)
    ...
    task_prompt_fusion_net= PromptFusionModule(768, output_dim=768)
    ...
    params_to_optimize.append({'params': feature_fusion_net.parameters(), 'lr': args.learning_rate})
    ...
    optimizer, train_dataloader, lr_scheduler, scb_net, img1_tpb_net,img2_tpb_net, feature_fusion_net,task_prompt_fusion_net= accelerator.prepare(optimizer,train_dataloader,
                                                                                                             lr_scheduler, scb_net, img1_tpb_net,
                                                                                                             img2_tpb_net,feature_fusion_net,task_prompt_fusion_net)
    ...
    feature_fusion_net.to(accelerator.device, dtype=weight_dtype)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with ((accelerator.accumulate(scb_net) and accelerator.accumulate(img1_tpb_net)and accelerator.accumulate(img2_tpb_net))and
                  accelerator.accumulate(task_prompt_fusion_net) and accelerator.accumulate(feature_fusion_net)):
                ...
                img1_normalized_pixel_values = torch.nn.functional.interpolate(img1_normalized_pixel_values, size=(224, 224), mode="bilinear", align_corners=False)
                ...
                # torch.save(img1_scb_cond,"img1.pt")
                # torch.save(img2_scb_cond,"img2.pt")
                # torch.save(fuse_scb_cond,"fuse.pt")
                # label_image = batch["label_values"].to(dtype=weight_dtype)
                # label_scb_cond = vae.config.scaling_factor * torch.chunk(vae.quant_conv(vae.encoder(label_image)), 2, dim=1)[0]
                # torch.save(label_scb_cond,"label.pt")
                # sys.exit()
                ...
                # pred = vae.decode(fuse_scb_cond / vae.config.scaling_factor, return_dict=False)[0]  # 3,512,512
                ...
                if accelerator.sync_gradients:
                    ...
                    accelerator.clip_grad_norm_(feature_fusion_net.parameters(), args.max_grad_norm)
                ...
                if accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0 or global_step == 1:
                        ...
                        if global_step != 1:
                            ...
                            save({'model': feature_fusion_net.state_dict()}, os.path.join(save_path, 'feature_fusion_net.pt'))
                            logger.info(f"Saved feature_fusion_net to {save_path}")
                    ...
                    if global_step % args.validation_steps == 0:
                        ...
                        log_validation(logger, vae, unet, image_encoder, scb_net, img1_tpb_net,img2_tpb_net,feature_fusion_net,
                                       task_prompt_fusion_net, args, accelerator, global_step)
