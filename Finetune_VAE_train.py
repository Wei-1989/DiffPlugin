...
from Finetune_VAE_val import log_validation
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--output_dir",type=str,default="./results/VAE/Finetune_VAE_0308a",)
...
def main(args):
    ...
    optimizer, train_dataloader, lr_scheduler,decoder= accelerator.prepare(optimizer,train_dataloader,lr_scheduler,decoder)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(vae.encoder):
                img_image = batch["img1_pixel_values"].to(dtype=weight_dtype)
                z,feature_list=encoder(img_image,return_feature=True)
                ...
                rec = decoder(z / vae.config.scaling_factor,feature_list)
                # 梯度损失 ----------------------------
                ...
                pixel_loss = F.mse_loss(img_image.float(), rec.float(), reduction="mean")
                color_loss= Color_Loss(img_image,rec)
                loss=pixel_loss+color_loss
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if (global_step % args.checkpointing_steps==0 and global_step >= 13000) or global_step == 1 or global_step == args.checkpointing_steps:
                    ...
                    if global_step % args.validation_steps == 0:
                        log_validation(logger, vae,encoder,decoder, args, accelerator, global_step)
                        encoder.train()
                        decoder.train()
