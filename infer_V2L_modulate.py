...
from modules import CustomEncoder,CustomDecoder,VAE_ShallowFeatureFusionModule, VAEFuseNet,TextAdapter,V2LModulate
from MyHook import *

def parse_args(input_args=None):
    parser = argparse.ArgumentParser(description="Simple example of a Diff-Plugin inference script.")
    parser.add_argument("--ckpt_dir", type=str, default="results/VIFusion/Hadamard_0401a/pick-checkpoint-13500/", required=False, )
    parser.add_argument("--VAE_ckpt_dir", type=str, default="results/Stage_two/whole_network/pick-checkpoint-54000/", required=False,)
    parser.add_argument("--v2lModulate_ckpt_dir", type=str, default="results/VIFusion/v2lModulate_0407a/checkpoint-13500/", required=False,)
    parser.add_argument("--img_path", type=str, default="./TestData")
    parser.add_argument("--text_type", type=str, default="category")
...
if __name__ == "__main__":
    ...
    # unet = UNet2DConditionModel.from_pretrained("./pretrained-large-modal/unet", subfolder="unet", revision=None).to(device)
    # unet.load_state_dict(torch.load(os.path.join(args.ckpt_dir, "unet.pt"),weights_only=True)['model'],strict=True)
    ...
    # my part---------------------
    v2lModulate=V2LModulate(2, clip_v_dim=1024,clip_text_dim=768).to(device)
    v2lModulate.load_state_dict(torch.load(os.path.join(args.v2lModulate_ckpt_dir, "v2lModulate.pt"),weights_only=True)['model'],strict=True)
    v2lModulate.eval()



    vae_shallow_feature_fusion_module=VAE_ShallowFeatureFusionModule().to(device)
    vae_shallow_feature_fusion_module.load_state_dict(torch.load(os.path.join(args.VAE_ckpt_dir, "feature_fusion_module.pt"),weights_only=True)['model'],strict=True)
    ...
    scb_net=SCBNet.from_unet(backup_unet, load_weights_from_unet=False).to(device)
    ...
    # 只要特定文件-----------------
    # target_filenames = {"00123D" }#"00123D","00196D","01458D"
    ...
    with torch.no_grad():
        ...
        for index,(img1,img2,text) in enumerate(zip(img1_images,img2_images,text_list)):
            ...
            img1_prompt_guidance = image_encoder(img1_normalized_pixel_values, output_attentions=True, output_hidden_states=True)

            img2_prompt_guidance = image_encoder(img2_normalized_pixel_values, output_attentions=True, output_hidden_states=True)


            fusion_prompt_guidance = v2lModulate(img1_prompt_guidance, img2_prompt_guidance)

            text_token = tokenizer(text, return_tensors="pt", padding=False, truncation=True).to(device=vae.device)
            decoded_text = tokenizer.decode(text_token['input_ids'][0], skip_special_tokens=True)
            text_features = text_encoder(**text_token, output_attentions=True, output_hidden_states=True).last_hidden_state

            visualization_tensor(fusion_prompt_guidance.mean(dim=1))
            visualization_tensor(text_features.mean(dim=1))
