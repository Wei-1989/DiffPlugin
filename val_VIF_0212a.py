...
def log_validation(logger, vae, unet, image_encoder,text_encoder,tokenizer, guidance_net, img1_tpb_net, img2_tpb_net,
                   text2image_prompt_fusion_net,
                   task_prompt_fusion_net, args, accelerator, step):
    logger.info("Running validation... ")

    guidance_net = accelerator.unwrap_model(guidance_net)
    ...
    image_logs = validation(args, vae, unet, image_encoder,text_encoder,tokenizer, guidance_net, img1_tpb_net, img2_tpb_net,
                            text2image_prompt_fusion_net,
...
def validation(args, vae, unet, image_encoder,text_encoder,tokenizer, guidance_net, img1_tpb_net, img2_tpb_net,
               text2image_prompt_fusion_net,
    ...
    text_folder = os.path.join(validation_images,'text_main_content')
    # 获取文件夹中的所有文件
    img1_image_path = [
    ...
    for index, (img1, img2,text) in enumerate(zip(img1_image_path, img2_image_path,text_path)):
        ...
        with torch.no_grad():
            ...
            for _, t in enumerate(timesteps):
                ...
                down_block_res_samples = guidance_net(img1_cond, img2_cond)
